...
- Workflow gọi PM2 qua binary local trong deploy dir (`D:\apps\phubai-mes\node_modules\.bin\pm2.cmd`).
- Workflow đặt `PM2_HOME` vào `D:\apps\phubai-mes\.pm2` (cố định, nằm ngoài thư mục checkout của runner).
- Build xảy ra trong `_work` (checkout dir), sau đó robocopy `/MIR` sang `D:\apps\phubai-mes`, PM2 chạy app từ đó. Tách biệt thư mục build và deploy để runner không bị lock file khi prepare workflow.
