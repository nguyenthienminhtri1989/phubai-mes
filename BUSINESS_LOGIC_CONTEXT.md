...
- Quy trình deploy PHUBAI-MES làm tương tự PHUBAI-ERP: Windows Server, PostgreSQL, Next.js production, PM2, Cloudflare Tunnel.
...
- Đường dẫn server mặc định trong `ecosystem.config.cjs`: `D:/PHUBAI-MES/phubai-mes`. Nếu clone sang thư mục khác phải sửa `cwd`.
- Trên server production chỉ dùng `npx prisma migrate deploy`, không dùng `prisma migrate dev`.
- Trước khi deploy server bằng `git pull`, code local phải được commit và push lên GitHub.
